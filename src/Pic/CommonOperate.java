...
public abstract class CommonOperate{
	 ...
	 * @throws IOException
	 */
	public Result findNearestValidRecord(String key,byte[] COL_NAME, Date timestamp)
			throws IOException {
		Result result = null;
		...
		if (timestamp.getMinutes() == 0 && timestamp.getMinutes() == 0) {
			...
			if (pre_result == null || pre_result.isEmpty()) {
				 //System.out.println("没有前一个时刻" + preTime);
				 
				if (next_result != null && !next_result.isEmpty() && Bytes.toInt(
						next_result.getValue(COLFAM_NAME, COL_NAME))!= -1) {
					result = next_result;
				}
			} else if (next_result == null || next_result.isEmpty()) {
				//System.out.println("没有后一个时刻" + nextTime);
				if (pre_result != null && !pre_result.isEmpty() && Bytes.toInt(
						pre_result.getValue(COLFAM_NAME, COL_NAME))!= -1) {
					result = pre_result;
				}
			} else {
				if (ts - preTime <= nextTime - ts) {
					//System.out.println("两者倾向前一个时刻");
					if(Bytes.toInt(pre_result.getValue(COLFAM_NAME, COL_NAME))!= -1){
						result = pre_result;
					}
					else if (Bytes.toInt(next_result.getValue(COLFAM_NAME, COL_NAME))!= -1) {
						result = next_result;
					}
					
				} else {
					 //System.out.println("两者倾向后一个时刻");
					if(Bytes.toInt(next_result.getValue(COLFAM_NAME, COL_NAME))!= -1){
						result = next_result;
					}
					else if (Bytes.toInt(pre_result.getValue(COLFAM_NAME, COL_NAME))!= -1) {
						result = pre_result;
					}
				}
